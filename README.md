...
除了基礎的員工資訊管理（CRUD）與 商品管理（CRUD)外，更使用了 **Interceptor** + **Filter** 雙重攔截驗證，以達成 RBAC權限控管 確保數據安全。本專案整合了 **WebSocket 即時通訊功能**，實現了員工與管理員之間的線上客服系統，且本專案結合完整的 **點數商城** 與 **數據可視化** 儀表板 以及 **日誌記錄**，為一個完整的SpringBoot+Vue+MySQL的專案。。
