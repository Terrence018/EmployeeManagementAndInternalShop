...
import { Timer, User, Monitor, Coin, View } from '@element-plus/icons-vue' // 補上 View icon
...
const handleTabClick = (tab) => {
  if (tab.props.name === 'operate') loadOperateData()
  else loadEmpLogData()
}
...
<template>
  <div style="padding: 20px;">
    <div style="margin-bottom: 20px;">
      ...
      <span style="color:#999; font-size:14px;">監控系統運行狀況與員工業務操作</span>
    </div>
        ...
        <el-tab-pane label="🔧 系統操作日誌" name="operate">
          <el-table :data="operateData" stripe v-loading="operateLoading">
            ...
            <el-table-column label="操作人" width="120">
              <template #default="scope">
                <el-tag effect="plain">
                   {{ scope.row.operateEmpName || '系統/未知' }}
                </el-tag>
              </template>
            </el-table-column>
          ...
          <div style="display:flex; justify-content:flex-end; margin-top:15px;">
            <el-pagination 
              ...
              :total="operateTotal" 
              v-model:current-page="operateParams.page"
        ...
        <el-tab-pane label="👤 員工業務日誌" name="emp">
          ...
          <div style="display:flex; justify-content:flex-end; margin-top:15px;">
            <el-pagination 
              layout="prev, pager, next" 
              :total="empLogTotal" 
              v-model:current-page="empLogParams.page"
              @current-change="handleEmpLogChange"
            />
          </div>
        </el-tab-pane>
    ...
    <el-dialog v-model="detailVisible" title="📜 日誌詳情" width="700px">
      <el-descriptions :column="1" border>
        ...
        <el-descriptions-item label="執行類名">
          {{ currentLog.className }}
        </el-descriptions-item>
        
        <el-descriptions-item label="執行方法">
        ...
        <el-descriptions-item label="操作時間">
          {{ currentLog.operateTime?.replace('T', ' ') }}
        </el-descriptions-item>
      </el-descriptions>
...
<style scoped>
/* 讓 JSON 數據顯示得好看一點，自動換行 */
.json-content {
  margin: 0;
  white-space: pre-wrap; /* 保留空格並自動換行 */
  word-break: break-all; /* 防止長單詞撐破容器 */
